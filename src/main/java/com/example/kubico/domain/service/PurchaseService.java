...
import com.example.kubico.infrastructure.dto.PurchaseCreateDTO;
import com.example.kubico.infrastructure.mapper.PurchaseMapper;
import com.example.kubico.infrastructure.persistence.PartRepository;
import com.example.kubico.infrastructure.persistence.PurchaseRepository;
...
public class PurchaseService {
  ...
  public Purchase create(PurchaseCreateDTO dto) {
    ...
    if (vehicle != null) {
      ...
      if (newQuantity == 0) {
        part.setStatus(
            com.example.kubico.domain.enums.PartStatus.OUT_OF_STOCK);
      }
  ...
  public Purchase confirmOrder(UUID id) {
    ...
    if (purchase.getVehicle() != null) {
      purchase.getVehicle().setStatus(
          com.example.kubico.domain.enums.VehicleStatus.SOLD);
  ...
  public Purchase cancelOrder(UUID id) {
    ...
    if (purchase.getVehicle() != null) {
      purchase.getVehicle().setStatus(
          com.example.kubico.domain.enums.VehicleStatus.FOR_SALE);
      vehicleRepository.save(purchase.getVehicle());
    } else if (purchase.getPart() != null) {
      ...
      purchase.getPart().setStatus(com.example.kubico.domain.enums.PartStatus.AVAILABLE);
