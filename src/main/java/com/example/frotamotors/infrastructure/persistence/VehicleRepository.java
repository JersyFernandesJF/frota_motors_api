  ...
  @Query(
      value =
          "SELECT v.* FROM vehicles v WHERE "
              ...
              + "(CAST(:startDate AS TIMESTAMP) IS NULL OR v.created_at >= CAST(:startDate AS TIMESTAMP)) AND "
              + "(CAST(:endDate AS TIMESTAMP) IS NULL OR v.created_at <= CAST(:endDate AS TIMESTAMP)) AND "
