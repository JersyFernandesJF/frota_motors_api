DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'vehicle_rentals') THEN
...
CREATE INDEX IF NOT EXISTS idx_rentals_renter ON vehicle_rentals(renter_id);
CREATE INDEX IF NOT EXISTS idx_rentals_agency ON vehicle_rentals(agency_id);
